...
# Fetch the team information from the teams table
...
def insert_Scorecard(teamID, courseName, roundType):
    ...
    try:
        ...
        if teamID: # if teamID exists
            queryInsert = f"Insert INTO scorecards (teamID, courseName, roundType) VALUES ('{teamID}', '{courseName}', '{roundType}')" # insert teamID and CourseName into scorecard
...
# Fetch the scorecard information from the scorecards table
...
# fetch scores from a scorecard
def fetch_scores(scorecardID):
    connection = connect_to_database()
    try:
        cursor = connection.cursor()
